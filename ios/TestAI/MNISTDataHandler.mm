...
RCT_EXPORT_METHOD(getLocalModelPathTwo : (RCTPromiseResolveBlock)resolve rejecter : (RCTPromiseRejectBlock)reject) {
  @try {
    NSString *modelPath = [[NSBundle mainBundle] pathForResource:@"model-2" ofType:@"onnx"];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if ([fileManager fileExistsAtPath:modelPath]) {
      resolve(modelPath);
    } else {
...
RCT_EXPORT_METHOD(getLocalModelPathQuan : (RCTPromiseResolveBlock)resolve rejecter : (RCTPromiseRejectBlock)reject) {
  @try {
    NSString *modelPath = [[NSBundle mainBundle] pathForResource:@"model_quantized" ofType:@"onnx"];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if ([fileManager fileExistsAtPath:modelPath]) {
      resolve(modelPath);
    } else {
...
RCT_EXPORT_METHOD(getImagePath : (RCTPromiseResolveBlock)resolve reject : (RCTPromiseRejectBlock)reject) {
  @try {
    ...
    if ([fileManager fileExistsAtPath:imagePath]) {
      ...
      reject(@"mnist", @"no such an image2", nil);
    }
  } @catch (NSException *exception) {
    reject(@"mnist", @"no such an image1", nil);
  }
}
